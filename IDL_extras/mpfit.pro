...
;      PARINFO[i].MPDERIV_DEBUG = 1   ; Enable derivative debugging
;      mode
;      PARINFO[i].MPDERIV_RELTOL = ?? ; Relative tolerance for
;      comparison
;      PARINFO[i].MPDERIV_ABSTOL = ?? ; Absolute tolerance for
;      comparison
...
;
;      PARINFO[i].MPDERIV_RELTOL = 1d-3 ;; Suggested relative
;      tolerance 
;      PARINFO[i].MPDERIV_ABSTOL = 1d-7 ;; Suggested absolute
;      tolerance
;
...
;    DIFF_REL
;    FJAC PARM 2
;               80    -0.7308    0.04233    0.04233 -5.543E-07
;               -1.309E-05
;               99      1.370    0.01417    0.01417 -5.518E-07
;               -3.895E-05
;              118    0.07187   -0.01400   -0.01400 -5.566E-07
;              3.977E-05
;              137      1.844   -0.04216   -0.04216 -5.589E-07
;              1.326E-05
;    FJAC DEBUG END
;
...
;             [ NOTE: the PARNAME can't be used in a TIED
;             expression. ]
;
...
;
;   START_PARAMS - An one-dimensional array of starting values for
;                  each of the
...
;             parameter values; however, the user function is
;             evaluated
...
;                 1 - if MIN_VERSION is set and MPFIT satisfies the
;                     minimum
;                 0 - if MIN_VERSION is set and MPFIT does not satisfy
;                     it
...
;
;   Minimizes sum of squares of MYFUNCT.  MYFUNCT is called with the
;   X,
...
;
;      f(x) = f(x0) + f'(x0) . (x-x0) + (1/2) (x-x0) . f''(x0)
;      . (x-x0)
;             -----   ---------------
;                   -------------------------------  (1)
;     Order    0th          1st                      2nd
...
;   It adds an additional diagonal term to the equation which may aid
;   the
...
;   factorization of h', where Q is an orthogonal matrix such
;   that QT .
...
;     Systems XVIII, Quebec, Canada, ASP Conference Series, Vol. XXX,
;     eds.
...
;       Link to ASP:
;       http://aspbooks.org/a/volumes/table_of_contents/411
;
...
;   Analytical derivatives allowed via AUTODERIVATIVE keyword, 09 Nov
;   1998
...
;   Changed definition of ITERPROC to be consistent with TNMIN, 19 Jan
;   1999 CM
...
;     to Huib Intema (who found it from the Python translation!), 05
;     Feb 2007
...
;   1.72-1.74 - Documented alternate method to compute correlation
;               matrix,
;          CM, 05 Feb 2010
...
;   1.83 - Trap more overflow conditions (ref. Nirmal Iyer), CM
;          2013-12-23
;   1.84 - More robust handling of FNORM, CM 2016-05-19
;
;  $Id: mpfit.pro,v 1.84 2016/05/19 16:07:44 cmarkwar Exp $
;-
; Original MINPACK by More' Garbow and Hillstrom, translated
; with permission
; Modifications and enhancements are:
; Copyright (C) 1997-2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010,
; 2011, 2012, 2013 Craig Markwardt
...
;  prof_start = systime(1)
  ...
  if keyword_set(autoderiv) EQ 0 OR max(dside[ifree] EQ 3) EQ 1 then begin
      ...
      if n_elements(fjac) NE m*nall then begin
          ...
          iflag = 1
;          profvals.fdjac2 = profvals.fdjac2 + (systime(1) -
;          prof_start)
          return, 0
       endif

      ;; This definition is consistent with CURVEFIT (WRONG, see
      ;; below)
  ...
  for j=0L, n-1 do begin
      ...
      if (dsidej EQ 3) AND (ddebugj EQ 1) then begin
          ...
          ;; If tolerances are zero, then any value for deriv triggers
          ;; print...
          if (da EQ 0 AND dr EQ 0) then $
            diffj = (fjaco NE 0 OR fjacj NE 0)
          ;; ... otherwise the difference must be a greater than
          ;; tolerance
...
;  prof_start = systime(1)
  ...
  x = r[delm]
  wa = qtb
  ;; Below may look strange, but it's so we can keep the right
  ;; precision
  zero = qtb[0]*0.
  half = zero + 0.5
  quart = zero + 0.25

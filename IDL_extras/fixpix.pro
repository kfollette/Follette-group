...
; a new bad pixel list by examining the image for pixels set to
; BADVAL.
...
if badcount le 0 then begin
                                ; already setoutimgs = imgs above, so
                                ; just return
   return
endif
...
;print,format='(" ",A)',strc(j)
  ...
  imoffset = j * sz(1) * sz(2)
                                ...
                                ; quickly deal with large contiguous
                                ; regions of bad pixels at the left

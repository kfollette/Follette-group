...
;
; PURPOSE:
;  Converts a string date in fits format  to double.  Is the same code from the date_conv function in the
;  astronomy library, but allows arbitrary precision in the seconds
;
; INPUTS:
;  datestr  : the input date in format 'YYYY-MM-DDTHH:MM:SS.SS...'
;
; INPUT KEYWORDS:
;  none
;
; OUTPUTS:
;  returns the date in double precision days
;
; HISTORY:
;  Written 2013-01-21 by Jared Males, jrmales@email.arizona.edu
;
;-
function fitsdate_conv,date,type
;+
; NAME:
;     DATE_CONV
; PURPOSE:
...
;
                  ...
                  ;handle old visao zulu time bug
                  if strlen(temp) eq 0 then begin
                        dtmp=gettok(day_of_month,'Z')
                        temp=day_of_month
                        day_of_month=dtmp
                  end
                  ...
                  hour = fix(gettok(temp,':'))
                  minute = fix(gettok(temp,':'))
                  sec = float(strtrim(strmid(temp,0)))
...
;
        ...
        'F' : begin
               xsec = strmid(string(sec+100,'(f10.6)'),1)

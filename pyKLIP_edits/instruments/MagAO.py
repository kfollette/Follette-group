   ...
   ###    Methods    ###
    ...
    def readdata(self, filepaths):
        ...
        for index, filepath in enumerate(filepaths):
            cube, center, pa, wv, astr_hdrs, filt_band, star_flux, prihdr = _magao_process_file(filepath, index)
            data.append(cube)
        ...
        #FILENUMS IS 1D LIST
        data = np.array(data)
        dims = data.shape #should be 2D (451 by 451 for us, but depending on chosen size)

...
;
; CALLING SEQUENCE:
;       atv [,array_name OR fits_file] [,min = min_value]
;[,max=max_value] 
...
;       header:     FITS image header (string array) for use with data
;array
;       
; OUTPUTS:
;       None.  
...
top_menu_desc = [ $
                ...
                {cw_pdmenu_s, 2, ' DSS'}, $
;                {cw_pdmenu_s, 2, ' FIRST'}, $  ;broken, don't
;                use for now
                {cw_pdmenu_s, 0, '--------------'}, $
                {cw_pdmenu_s, 2, 'Quit'}, $
...
; endpoints and plot Vector Distance vs. Pixel Value with
; atv_vectorplot
...
; Make the 'pan' image that shows a miniature version of the full
; image.
...
; If pixel table widget is open, update pixel values and cursor
; position
if (xregistered('atv_pixtable', /noshow)) then atv_pixtable_update


; Prevent the cursor move from causing a mouse event in the draw
; window
...
;Redisplay inverted image with current zoom, update pan, and refresh
;image
atv_displayall

;make sure that the image arrays are updated for line/column plots,
;etc.
atv_resetwindow
...
; you expect (especially in log-scaling mode), but mostly it works
; fine.
...
; Check and adjust the zcenter if the cursor is near the edges of the
; image
...
;               (instrume EQ 'LRIS' or instrume EQ 'LRISBLUE')) then
;               begin
...
; fits reader for fpack-compressed .fz images with no extensions.
; Note: uses
...
; converts ra and dec strings in hh:mm:ss and dd:mm:ss to decimal
; degrees
...
; make sure that we don't keep the cmps_form window as the
; active window
...
; routine to change color stretch for given values of brightness and
; contrast.
...
case tablename of
    ...
    'SAURON': begin
                                ; This is the 'SAURON' color map by
...
; This program calculates the pixel scale (deg/pixel) and returns the
; value
...
; function to convert an ATV region from wcs coordinates to pixel
; coordinates
    ...
    case strlowcase(reg_type) of
        ...
        'circle': begin
            ...
            radius = float(coords_arr[2]) * state.zoom_factor
                                ...
                                ; added by AJB: rescale for postscript
                                ; output for each plot type
...
; The following allows for 2 conditions, depending upon whether X and
; Y
; are set
...
;       if ptr_valid(state.reg_ids_ptr) then ptr_free,
;       state.reg_ids_ptr
;       widget_control, event.top, /destroy
...
;    if (state.wcstype EQ 'angle') then begin
;        xy2ad, state.coord[0], state.coord[1], *(state.astr_ptr),
;lon, lat
;        
;        wcsstring = atv_wcsstring(lon, lat, (*state.astr_ptr).ctype,
;$
;                                  state.equinox,
;state.display_coord_sys, $
;                                  state.display_equinox,
;state.display_base60)
;        ;;
;
;        if (strpos(wcsstring, 'J2000') ne -1) then coord_select = 1
;        if (strpos(wcsstring, 'B1950') ne -1) then coord_select = 2
;        if (strpos(wcsstring, 'Galactic') ne -1) then coord_select =
;3
;        if (strpos(wcsstring, 'Ecliptic') ne -1) then coord_select =
;4
;        
;        if (strpos(wcsstring, 'J2000') eq -1 AND $
;            strpos(wcsstring, 'B1950') eq -1 AND $
;            strpos(wcsstring, 'Galactic') eq -1 AND $
;            strpos(wcsstring, 'Ecliptic') eq -1) then coord_select =
;5
;        
...
;        widget_control,(*state.reg_ids_ptr)[2],
;set_droplist_select=coord_select
;    endif    
;    
;endif else begin
...
; when histplot window is not already present or plot window is
; present
...
; Bug fix 4/16/2000: added call to round to make sure bigbox is an
; integer
...
; extract relevant image subset (may be rectangular), translate coord
; origin,
...
; now loop through photometry radii, finding the total flux,
; differential
...
; TO DO: future update, can eliminate a lot of this image scaling
; stuff
...
for i = xspec[0], max(xspec) do begin
   ...
   j = i - xspec[0]
                                ; error check to see if spectrum runs
                                ; off the top or bottom of image

...
"""
# Actual usage:
# python pradialprofile.py image parallax(mas) pixelscale(arcsec/px) filter AbsMag ScaleFactor SatExptime UnsatExptime PeakUnsatFlux

Modified version of the original contrast curve code from 2015 MinMs paper (for VLT/CFHT/MMT). 
Version here specifically designed for saturated MMT data to estimate limiting magnitude of the 
background.
...
filter
AbsMag
ScaleFactor - scaling of transmission between the narrow and broadband filters
SatExptime
UnsatExptime
PeakUnsatFlux - counts in peak pixel from the unsaturated image

In progress:
- adding scaling factor to account for narrowband to broadband saturated images
- have the script use both the unsaturated and saturated images to derive the peak pixel count in unsaturated frame

Last updated: 
2020-04-06 - (KWD) Started code - modified from Unsat_ContrastCurve.py
...
# use defined header position for saturated psf
...
# Old Unsaturated version: Estimate peak pixel count 
...
peak = (SatExptime/UnsatExptime)*ScaleFactor*PeakUnsatFlux

print(f"The estimated peak pixel count is {peak}")

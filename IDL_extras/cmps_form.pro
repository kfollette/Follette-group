...
;
;   Based almost entirely on, but a totally revamped version of,
;   CMPS_FORM by 
;   FANNING SOFTWARE CONSULTING (David Fanning Ph.D.)
...
;   Keywords appropriate for the PostScript DEVICE command are
;   returned.
;
;   Use your LEFT mouse button to move the "Plot Window" around the
;   page.
;   Use your RIGHT mouse button to draw your own "Plot Window" on the
;   page.
;
; HELP:
...
; OPTIONAL INPUTS:
;
;    XOFFSET -- Optional xoffset of the top-level base of
;               cmps_form. Default is
;    to try to center the form on the display.
;
;    YOFFSET -- Optional yoffset of the top-level base of
;               cmps_form. Default is
;    to try to center the form on the display.
;
; INPUT KEYWORD PARAMETERS:
;
;    BITS_PER_PIXEL -- The initial configuration of the bits per pixel
;                      button.
;
;    BLOCKING -- Set this keyword to make this a blocking widget under
;                IDL 5.0.
;    (All widget programs block under IDL 4.0.)
;
;    COLOR -- The initial configuration of the color switch.
;
;    DEFAULTS -- A stucture variable of the same type and structure as
;                the
;    RETURN VALUE of cmps_form. It will set initial conditions. This
;    makes
;    it possible to start cmps_form up again with the same values it
;    had the
...
;
;    ENCAPSULATED -- The initial configuration of the encapsulated
;                    switch.
;
...
;
;    INITIALIZE -- If this keyword is set, the program immediately
;                  returns the
;    "localdefaults" structure. This gives you the means to configue
;    the
...
;              be returned by cmps_form when INITIALIZE is set.  This
;              is
;              a convenient way to select a predefined config
;              non-interactively.
;
;    LANDSCAPE -- The initial configuration of the landscape/portrait
;                 switch.
;
;    LOCALDEFAULTS -- A structure like the DEFAULTS structure. If
;                     specified,
;    then it is added as a predefined configuration entry called
;    "Local".
...
;                 'Letter'.
;
;    MARGINSIZE -- Size of the margins on all sides.  Default is 0.25
;                  inches.
;                  When MARGINSIZE is non-zero, a graphic cannot
;                  directly
;                  abut the edge of the page.  This is normally a good
;                  thing,
;                  since there is often a  non-printable region which
;                  borders
;                  the page.
;
...
;
;    CANCEL -- This is an OUTPUT keyword. It is used to check if the
;              user
;    selected the "Cancel" button on the form. Check this variable
;    rather
;    than the return value of the function, since the return value is
;    designed
;    to be sent directly to the DEVICE procedure. The varible is set
;    to 1 if
;    the user selected the "Cancel" button. Otherwise, it is set to 0.
;
;    CREATE -- This output keyword can be used to determine if the
;              user
;    selected the 'Create File' button rather than the 'Accept'
;    button.
...
;                  bits_per_pixel:0, $ ; How many bits per image
;                  pixel?
;                  encapsulated:0,$    ; Encapsulated or regular
;                  PostScript?
...
;     - Option of storing more than one predefined postscript
;       configuration
...
;   Fix for IDL 5.5's handling of structures with arrays, CM,
;   11 Dec 2001
;   Replaced obsolete PICKFILE call with DIALOG_PICKFILE, Jeff
;   Guerber,
;     24 Sep 2004
;   Transfer DEFAULTS and LOCALDEFAULTS values via
;   STRUCT_ASSIGN,/NOZERO
;     instead of EXECUTE, Jeff Guerber, 24 Sep 2004.
;   Set CANCELBUTTON and CREATEBUTTON immediately on entry, so they're
;     defined even if user kills the window, Jeff Guerber, 24 Sep
;     2004.
;
; COMMON BLOCKS:
; 
...
;
;        cmps_form_DEFAULT_PAPERSIZE - a string designating the
;                                      default
...
;
;        cmps_form_STDCONFIGS - An array of cmps_form_CONFIG
;                               structures,
...
;                             configuration.
;
;   See the IDL source code cmps_form_LOAD_CONFIGS for an example of
;   how
...
;   procedure cmps_form_LOAD_CONFIGS.  Any modifications that take
;   place
...
;           Letter Legal Tabloid Ledger Executive Monarch Statement
;           Folio
...
pro cmps_form_load_configs, defaultpaper, configs
  ...
  ;   1.  7x5    inch color plot region in portrait
                                ;   2.  7.5x10 inch centered color
                                ;   plot region, covering almost whole
  ;                   portrait page (0.5 inch margins)
                                ;   3.  10x7.5 inch centered color
                                ;   plot region, covering almost whole
  ;                   landscape page (0.5 inch margins)
                                ;   4.  7x5    inch gray plot region
                                ;   in portrait (IDL default config)
...
; clearing house for self-consistent modification of the info
; structure.
;
; INPUTS
...
; cmps_form_DRAW_FORM
;
; Update the widget elements of the cmps_form form, using the INFO
; structure.
;
...
pro cmps_form_draw_form, info, nobox=nobox
  ...
  if string(wfilename(0)) NE info.devconfig.filename then begin
      Widget_Control, info.idfilename, Set_Value=info.devconfig.filename
                                ; Put caret at end of pathname text so
                                ; that filename itself is visible
...
Pro cmps_form_Move_Box, event
   ...
   ; event handler back to "cmps_form_Box_Events" when it senses an
                                ; "UP" draw button event and it will
                                ; also turn
                                ; cmps_form_Draw_Motion_Events
   ; OFF.
  ...
  ; ixmax and iymax are the max values of x and y, in pixels
                                ...
                                ; info.ideltx/y contains the offset of
  ...
  IF whatButtonType EQ 'UP' THEN Begin
                                ...
                                ; When the button is "up" the moving
                                ; event is over.  We reset the
                                ; event function and update the
                                ; information about the box's position
...
Pro cmps_form_Grow_Box, event
   ...
   ; until an UP event is detected. Then it will set the event handler
                                ; back to cmps_form_Box_Events and
                                ; turn cmps_form_Draw_Motion_Events to
                                ; OFF.
...
; specialized procedures cmps_form_grow_box and cmps_form_move_box
; above.
;
  ...
  CASE whatButtonPressed OF
      ...
      'RIGHT': Begin
         ...
         ; Resize the plot box interactively. Change the event handler
                                ; to cmps_form_Grow_Box. All
                                ; subsequent events will be handled by
                                ; cmps_form_Grow_Box until an UP event
                                ; is detected. Then you will
      ...
      'LEFT': Begin
         ...
         ; Resize the plot box interactively. Change the event handler
                                ; to cmps_form_Move_Box. All
                                ; subsequent events will be handled by
                                ; cmps_form_Move_Box until an UP event
                                ; is detected. Then you will
         ; return to this  event handler. Also, turn motion events ON.
...
pro cmps_form_predef_events, event
  ...
  info.marginsize = 0.25 * convfactor
                                ...
                                ; The conversion between length and
                                ; pixels cannot always be set
                                ; precisely,
...
Function cmps_form, xoffset, yoffset, Cancel=cancelButton, Help=help, $
     ...
     xpagesize=xpagesize, ypagesize=ypagesize, pagebox=pagebox
                                ...
                                ; If the Help keyword is set, print
                                ; some help information and return
  ...
  IF Keyword_Set(help) THEN BEGIN
   ...
   ENDIF
                                ...
                                ; Set cancelButton and createButton as
                                ; if canceled, so will be defined
                                ; (and with appropriate values) even
                                ; if user kills the window instead of
  ...
  if N_Elements(marginsize) EQ 0 then $
    marginsize    = 0.25 * convfactor ; 1/4 inch margins "minimum"
                                ...
                                ; "Unconvert" the configuration xoff,
                                ; yoff, etc. into human-readable
                                ; format,
                                ; which is also the format of the
                                ; keywords xoff and yoff passed to
                                ; cmps_form()
  ...
  IF Keyword_Set(initialize) THEN BEGIN
   ...
   ; This program cannot work if the graphics device is already set
                                ; to PostScript. So if it is, set it
                                ; to the native OS graphics device.
  ...
  IF !D.Name EQ 'PS' THEN BEGIN
   ...
   ENDIF ELSE oldName = !D.Name
                                ...
                                ; Check for optional offset parameters
                                ; and give defaults if not passed
...
;   bitslabel = Widget_Label(opttlb, Value='   Color Bits:')
   ...
   textlabel = Widget_Label(fbase, Value='Filename: ')
                                ...
                                ; Set up text widget with an event
                                ; handler that ignores any event.
   ...
   filenameb = widget_button(fbase, value='Choose...', $
                             event_pro='cmps_form_select_file')
                                ...
                                ; This is a base for selection of
                                ; predefined configurations and paper
                                ; sizes
   ...
   pixwid = !D.Window
                                ...
                                ; Create a handle.  This will hold the
                                ; result after the widget finishes
   ptr = Handle_Create()
   ...
   Widget_Control, tlb0, Set_UValue=info, /No_Copy
                                ...
                                ; Set this widget program up as a
                                ; modal or blocking widget. What this
                                ; means
                                ; is that you will return to the line
                                ; after this XManager call when the
   ; widget is destroyed.

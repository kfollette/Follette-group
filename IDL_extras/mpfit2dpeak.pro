...
;      A[1]   Peak value
;      A[2]   Peak half-width (x) -- gaussian sigma or half-width at
;                                    half-max
;      A[3]   Peak half-width (y) -- gaussian sigma or half-width at
;                                    half-max
;      A[4]   Peak centroid (x)
;      A[5]   Peak centroid (y)
...
;
;              DOF     = N_ELEMENTS(Z) - N_ELEMENTS(A)   ; deg of
;                                        freedom
...
;   Oct 2006
;
;  $Id: mpfit2dpeak.pro,v 1.8 2006/10/09 19:25:29 craigm Exp $
;-
...
function mpfit2dpeak, z, a, x, y, estimates=est, tilt=tilt, $
  ...
  if n_elements(est) EQ 0 then begin
     ;; Here is the secret - the width is estimated based on the
     ;;                      volume
